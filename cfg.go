...
func (c *TfcatConfig) DefineFlags(fs *flag.FlagSet) {
	...
	fs.IntVar(&c.RawCount, "rawskip", 0, "count of raw messages to skip before passing the rest to stdout (-p, -s, and -f are ignored if -rawskip is given). -rawskip is useful for extracting messages from the middle of a file. -raw count of messages (after the skip count) are written to stdout if both -raw and -rawskip are given.")
