...
func (c *TfcatConfig) DefineFlags(fs *flag.FlagSet) {
	...
	fs.IntVar(&c.RawCount, "rawskip", 0, "count of raw messages to skip before passing the rest to stdout (-p, -s, -f, and -raw are ignored if -rawskip is given). -rawskip is useful for extracting messages from the middle or end of a file.")
